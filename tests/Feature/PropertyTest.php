...
class PropertyTest extends TestCase
{
    ...
    public function a_property_can_be_created()
    {
        ...
        Storage::disk('public')->assertMissing('filepond/tmp/'.$response1->content());
        Storage::disk('public')->assertMissing('filepond/tmp/'.$response2->content());
        Storage::disk('public')->assertMissing('filepond/tmp/'.$response3->content());
        Storage::disk('public')->assertMissing('filepond/tmp/'.$response4->content());
        ...
        Storage::disk('public')->assertExists($response1path);
        Storage::disk('public')->assertExists($response2path);
        Storage::disk('public')->assertExists($response3path);
        Storage::disk('public')->assertExists($response4path);
        
        $this->assertDatabaseHas('property_media', [
                                                    'property_id' => $property->property_id, 
                                                    'path' => $response1path,
                                                    'mime_type' => Storage::disk('public')->mimeType($response1path),
                                                    'extension' => pathinfo(storage_path('app/public').$response1path, PATHINFO_EXTENSION),
                                                    'size_in_bytes' => Storage::disk('public')->size($response1path),
                                                    'formatted_size' => CustomMethods::formatSizeUnits(Storage::disk('public')->size($response1path)),
                                                ]);                         
        $this->assertDatabaseHas('property_media', [
                                                    'property_id' => $property->property_id, 
                                                    'path' => $response2path,
                                                    'mime_type' => Storage::disk('public')->mimeType($response2path),
                                                    'extension' => pathinfo(storage_path('app/public').$response2path, PATHINFO_EXTENSION),
                                                    'size_in_bytes' => Storage::disk('public')->size($response2path),
                                                    'formatted_size' => CustomMethods::formatSizeUnits(Storage::disk('public')->size($response2path)),
                                                ]);                         
        $this->assertDatabaseHas('property_media', [
                                                    'property_id' => $property->property_id, 
                                                    'path' => $response3path,
                                                    'mime_type' => Storage::disk('public')->mimeType($response3path),
                                                    'extension' => pathinfo(storage_path('app/public').$response3path, PATHINFO_EXTENSION),
                                                    'size_in_bytes' => Storage::disk('public')->size($response3path),
                                                    'formatted_size' => CustomMethods::formatSizeUnits(Storage::disk('public')->size($response3path)),
                                                ]);                         
        $this->assertDatabaseHas('property_media', [
                                                    'property_id' => $property->property_id, 
                                                    'path' => $response4path,
                                                    'mime_type' => Storage::disk('public')->mimeType($response4path),
                                                    'extension' => pathinfo(storage_path('app/public').$response4path, PATHINFO_EXTENSION),
                                                    'size_in_bytes' => Storage::disk('public')->size($response4path),
                                                    'formatted_size' => CustomMethods::formatSizeUnits(Storage::disk('public')->size($response4path)),
                                                ]);                         
    }
